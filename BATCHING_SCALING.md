...
Sub-batches are **4-slot** (half the ship default). Aggregators are **wrapper_64** (up to 4 inputs per aggregator), giving a **flatter tree** than binary wrapper_16.

```
16 × sub-batch[4] ──► wrapper[noir-recursive] each      (leaf level)
    ↓ 4 at a time
4 × wrapper_64                                          (level 1)
    ↓ 4 at a time
1 × wrapper_64                                          (level 2, final)
    ↓
submit_batch_16 (L3 contract)
    ↓
settle_batch_16@batch=N (single public call)
```
...
- Public call stack enqueues **1 × `settle_batch_16`** at batch=N sizing.
- Nonce advances by 1.
- **Circuit work**: need a `wrapper_64` circuit (analogous to `wrapper_16` but takes 4 wrapper proofs + 4 settle-data re-feeds). For non-power-of-4 leaf counts, accept active-input bitmasks OR fall back to `wrapper_16` at the root.

### Path B2 — Recursive with large sub-batches + binary aggregators (current)

The pattern currently shipped. Sub-batches are **8-slot**. Aggregators are **wrapper_16** (2 inputs per aggregator), giving a **binary tree** of depth `log₂(K)`.

```
K × sub-batch[8] ──► wrapper[noir-recursive] each         (leaf level)
    ↓ 2 at a time
K/2 × wrapper_16                                         (level 1)
    ↓ 2 at a time
K/4 × wrapper_16                                         (level 2)
    ↓ ...
1 × wrapper_16                                           (root)
    ↓
submit_batch_16
    ↓
settle_batch_16@batch=N
```

- **Sub-batch size: 8** — as shipped.
- **Aggregator fan-in: 2** — wrapper_16, as shipped.
- **Sub-batches per L2 tx: K = N / 8.**
- Number of aggregator proves: **K - 1**.
- L2 tx: 1 merged proof + batch=N settle arrays (same as B1).
- Private circuit: 1 × `verify_honk_proof`.
- Public execution: 1 × `settle_batch_16@batch=N`.
- **Circuit work**: the `wrapper_16` shipped today already works; just drive it recursively. Need tree-level variants (wrapper_16 over wrappers, wrapper_16 over already-merged proofs of larger batch sizes) OR one generalized circuit.
...
| `wrapper_16` prove | ~60 s, ~3 GiB peak |
| `pair_tube` prove (RollupHonk, 519-field inner proofs) | ~65 s, ~4 GiB peak (step10 measured) |
| `wrapper_64` prove (estimated: 2× wrapper_16 verify work) | ~120 s, ~5-6 GiB peak |
...
| Aggregator layout | none (4 tube proofs direct to L2) | 2 × wrapper_64 → 1 wrapper_16 | 2 × wrapper_16 → 1 wrapper_16 |
...
| **Public execution** | 4 × `settle_batch@batch=8` | 1 × `settle_batch_16@batch=32` | 1 × `settle_batch_16@batch=32` |
...
| Circuit changes | 0 | **new wrapper_64**; recompile batch_app_standalone at batch=4 | generalize wrapper_16 across tree levels (or compile multiple variants) |
| Contract changes | `submit_four_batches` (trivial extension) | resize `submit_batch_16` + `settle_batch_16` to batch=32 | same as B1 |
...
| Aggregator layout | none (16 tube proofs direct to L2) | L1: 8 × wrapper_64; L2: 2 × wrapper_64; L3: 1 × wrapper_16 | L1: 8; L2: 4; L3: 2; L4: 1 (all wrapper_16) |
...
| **Public execution** | 16 × `settle_batch@batch=8` | 1 × `settle_batch_16@batch=128` | same |
...
| Circuit changes | 0 | new wrapper_64; 3 tree-level variants OR 1 parameterized | wrapper_16 at 4 tree-level variants OR 1 parameterized |
...
- **The single `settle_batch_16@batch=128` public call is the next wall.** ~128 storage reads + writes for deposits + ~128 for withdrawals + 2×256-iteration count loops. Close to the per-tx mana ceiling. A skeleton-proof dry run is strongly advised before committing to a full real-proof run.
...
| If you might hit the `settle_batch_16` mana ceiling | Verify with a skeleton-proof tx first | The ceiling applies to both B1 and B2 at large N |
...
3. **Write `wrapper_64`**. Input: `wrapper_vk`, 4 × (wrapper_proof, wrapper_public_inputs), 4 × (nullifiers[8], note_hashes[8], deposits[4], withdrawals[4]). Output: the 8 BatchOutput fields over merged arrays (32+32+16+16 for 4 sub-batches of size 4).
4. **Tree-level variants of wrapper_64** for non-leaf levels: `wrapper_64_L2` aggregates 4 level-1 merged outputs (each a 16-slot merged batch) into a 64-slot merged batch; `wrapper_64_L3` aggregates 4 level-2 → 256-slot. For **32-slot**: level-1 wrapper_64 + one wrapper_16 at root (reusing today's wrapper_16 at its current sizing). For **128-slot**: level-1 quad, level-2 quad, level-3 wrapper_16. Ideal: parameterize or use const-generics to avoid writing multiple crates.
5. **Resize `contract_recursive`**: batch-128 arrays in `submit_batch_16_large` + `settle_batch_16_large` (or extend `settle_batch_16` with configurable sizes — Noir doesn't support runtime-variable arrays, so compile-time variants).
...
### Path B2 (binary-tree scaling on the existing wrapper_16)

1. **Generalize wrapper_16** to work at any tree level: take two merged proofs of the same size and concatenate. Requires either const-generics or multiple compiled variants (e.g., `wrapper_16_8`, `wrapper_16_16`, `wrapper_16_32`, `wrapper_16_64`).
2. **Resize `contract_recursive`** to batch=128 (256/256/128/128 arrays).
3. **Harness**: recursive tree driver over wrapper_16 calls, managing the 2-concurrent leaves that fit in 16 GiB.

**Effort**: ~3-4 days (wrapper_16 level variants, contract resize, harness driver).

**Advantage over B1**: reuses the wrapper_16 circuit already in the repo; no new circuit type. Fewer aggregator proves at the same N due to the binary tree having more leaves but smaller aggregators (quad is bigger per prove than pair, so total aggregator work is similar).
...
| Circuit work | 0 | medium (wrapper_64 + variants) | medium (wrapper_16 variants) | 0 | medium | medium |
...
A fourth path that combines IVC sub-batch proving (~4 GiB each, highly concurrent) with `pair_tube` RollupHonk aggregation. Uses `verify_rolluphonk_proof` to verify tube proofs (519-field RollupHonk with IPA material) inside a Noir circuit — something `verify_honk_proof` (used by wrapper_16) cannot do.
...
| Contract changes | resize `settle_batch_16` to batch=32 |
...
Path C's advantage scales with the number of sub-batches: IVC leaves at ~4 GiB allow 4 concurrent on 16 GiB (vs B2's 2 concurrent at ~8 GiB each), and the pair_tube aggregation overhead (~10% slower per prove than wrapper_16) is negligible compared to the leaf-level speedup.
