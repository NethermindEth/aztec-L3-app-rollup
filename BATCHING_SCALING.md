...
| Contract changes | `submit_four_batches` (trivial extension) | resize `submit_merged_batch` + `settle_batch_16` to batch=32 | same as B1 |
...
5. **Resize `contract_recursive`**: batch-128 arrays in `submit_merged_batch_large` + `settle_batch_16_large` (or extend `settle_batch_16` with configurable sizes — Noir doesn't support runtime-variable arrays, so compile-time variants).
