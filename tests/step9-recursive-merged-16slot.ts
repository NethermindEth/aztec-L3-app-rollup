 ...
 *
 * Recursive pipeline with proof aggregation via pair_wrapper.
 *
 * Structure:
...
async function main() {
  console.log("=== step9: Recursive pipeline with pair_wrapper merged-proof ===\n");
  ...
  // and pair_wrapper prove. Sub-batch proving uses apiA / apiB.
  const api = await Barretenberg.new({ threads: 4 });

  console.log("Computing wrapper + pair_wrapper VK hashes...");
  const vkStart = performance.now();
  const { vkHash: wrapperVkHash } = await computeWrapperVkHash(api);
  const { vkHash: pairVkHash } = await computePairWrapperVkHash(api);
  console.log(`  wrapper VK hash: ${wrapperVkHash.toString().slice(0, 18)}...`);
  console.log(`  pair_wrapper VK hash: ${pairVkHash.toString().slice(0, 18)}... (${fmt(performance.now() - vkStart)})\n`);
  ...
  const { contract: l3 } = await Contract.deploy(
    wallet, l3Artifact,
    [initialStateRoot.toBigInt(), wrapperVkHash.toBigInt(), pairVkHash.toBigInt()],
    "constructor",
  ).send({ from: admin });
  ...
  // Sub-batch proving with wrapperTarget=noir-recursive for pair_wrapper.
  //
  ...
  console.log("Building two RECURSIVE sub-batches SEQUENTIALLY (wrapper at noir-recursive target)...");

  const subBatchStart = performance.now();
  const subBatchAStart = performance.now();
  const artifactA = await buildBatchProofRecursive(
    api, l3State, [depProofA], { wrapperTarget: "noir-recursive" },
  );
  ...
  const artifactB = await buildBatchProofRecursive(
    api, l3StateB, [depProofB], { wrapperTarget: "noir-recursive" },
  );
  ...
  // pair_wrapper prove (serial).
  console.log("Running pair_wrapper aggregation (serial)...");
  ...
  console.log(`  pair_wrapper prove: ${fmt(pairMs)}`);
  ...
  const metrics = {
    ...
    l2: {
      ...
      note: "submit_merged_batch enqueues ONE settle_batch_merged public call",
    },
    proofSizes: {
      pairWrapperProofBytes: pairArtifact.pairProof.length,
      pairWrapperVkBytes: pairArtifact.pairVk.length,
      wrapperProofBytesPerSubBatch: artifactA.tubeProof.length,
    },
    verificationReasoning: "Private circuit contains 1 × verify_honk_proof (of pair_wrapper). pair_wrapper itself contains 2 × verify_honk_proof (of wrapper) — this cost is paid during PROVING, not during L2 verification.",
  ...
  console.log("\n=== step9 SUMMARY ===\n");
  console.log(`  Variant: Recursive merged via pair_wrapper + submit_merged_batch`);
  ...
  console.log(`  Private verify cost:        1 × verify_honk_proof(pair_wrapper) in kernel`);
  console.log(`  Public execution cost:      1 × settle_batch_merged@batch=16`);
  console.log(`  Final on-chain proof size:  ${pairArtifact.pairProof.length} bytes (identical to wrapper)`);
